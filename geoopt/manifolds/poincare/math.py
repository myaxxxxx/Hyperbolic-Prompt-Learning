...
###################################################################
# import sys
# sys.path.append(r'/home/jjchen/LST/Hype_Embed/poincare-embeddings')
...
BALL_EPS = {torch.float32: 1e-5, torch.float64: 1e-10}

def tanh(x):
    # return x.clamp(-15, 15).tanh()
    return x.clamp(-5, 5).tanh()
...
def _dist(x, y, c, keepdim: bool = False, dim: int = -1):
    sqrt_c = c ** 0.5
    # I add this, in case the result of mobius_add extends the ball
    m_add = _mobius_add(-x, y, c, dim=dim).norm(dim=dim, p=2, keepdim=keepdim)
    if m_add > c:
        m_add = project(_mobius_add(-x, y, c, dim=dim)).norm(dim=dim, p=2, keepdim=keepdim)
    # I add this, in case the result of mobius_add extends the ball
    dist_c = artanh(
        sqrt_c * m_add
    )
    return dist_c * 2 / sqrt_c
    # return PM().distance(x, y)
